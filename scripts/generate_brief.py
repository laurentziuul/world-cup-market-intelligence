...
TEMPLATE_NAME = "daily_brief.md.j2"
...
def normalize_snapshot(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["volume_change_display"] = df["volume_change_24h"].apply(format_number)
    df["liquidity_change_display"] = df["liquidity_change_24h"].apply(format_number)
...
def get_brief_date(df: pd.DataFrame) -> str:
    if "snapshot_time_utc" not in df.columns or df.empty:
        return datetime.now(timezone.utc).date().isoformat()

    raw_snapshot_time = str(df["snapshot_time_utc"].iloc[0])

    try:
        parsed = pd.to_datetime(raw_snapshot_time, utc=True)
...
def build_context(df: pd.DataFrame) -> dict:
    ...
    if total_volume == 0 and total_liquidity == 0:
        liquidity_note = (
            "Volume and liquidity are currently zero because the pipeline is using "
    ...
    red_team_notes = [
        ...
        "Short trend history can confuse repeated manual tests with persistent market movement.",
        "Without multiple providers, there is no cross-market confirmation.",
    ]

    interpretation_notes = [
        "24h movers show the latest snapshot-to-snapshot change.",
        "Historical trends show movement across the full local snapshot archive.",
        "Trend quality is weak until there are enough observations and a meaningful total move.",
        "A strong narrative without liquidity remains a hypothesis, not a confirmed market signal.",
    ]
    ...
    context = {
        ...
        "trends_path": str(TRENDS_PATH.relative_to(ROOT)),
...
def main() -> None:
    ...
    output_path = OUTPUT_DIR / f"{context['brief_date']}-world-cup-market-brief.md"
